...
class BCQNetwork:
	def __init__(self, name, state_dim, action_dim, max_action, actor_hs_list, actor_lr, critic_hs_list, critic_lr, dqda_clipping=None, clip_norm=False):
		...
		with tf.variable_scope(self.name):
			...
			#print("soft q shape is ", self.soft_q_)
			...
			#print("soft q max shape is ", self.soft_q_max_)
			self.target_q_ = self.reward_ + self.flipped_done_ * self.discount_ *self.soft_q_max_ #next_state estimate is 0 if done
			#print("target q shape is ", self.target_q_)

			#train critic with combined losses of both critic network heads
			#print("testing shapes ofr target and critic_1_out ", self.target_, self.critic_1_out_, self.critic_2_out_)
...
class VAE(object):
	def __init__(self, state_dim, action_dim, latent_dim, max_action, vae_lr):
		...
		#print("shape is ", self.std_.get_shape())
		#print("shape is ", self.std_.get_shape().as_list())
		...
		#print("testing std and tf random normal sizes")
		#print("self.std_ shape, self.std_, self.epsilon_", self.std_.get_shape(), self.std_, self.epsilon_)
		#print("z", self.z_)
		# decoder
...
class BCQ(object):
	...
	def select_action(self, state):
		...
		#print("testing shapes in select action. tile state and z ", np.shape(tile_state), np.shape(z) )
		...
		# print("test shapes for select action")
		# print("tiled state shape", np.shape(tile_state))
		# print("action index shape and value", np.shape(action_index), action_index)
		# print("bcq_action shape and value ", np.shape(bcq_action), bcq_action)
		# print("q1 values and selected value", q1, q1[np.argmax(q1)])
		# print("outputted action is ", bcq_action[action_index])
		# return that index of action, not a prob distribution
